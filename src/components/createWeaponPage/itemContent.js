...
function ItemContent(props){
    ...
    const handleBuy = async() => {
        if(wallet.status === "connected"){
            ...
            if(Number(allowance.toString())<Number(eggPrice.toString())) {
                var tx = await signedAtariToken.approve(signedWeaponNFT.address,eggPrice.mul(1000))
                    .catch((err)=>{
    ...
    const buyEgg = async() => {
        ...
        .catch((err)=>{
            ...
            setAlertInfos({title:"Buy Error",info:"Please check AtariToken balance!"});
            setAlertOpen(true);
            setLoading(false);
        })
        if(tx!=null){
    ...
    return(
        ...
        {isReady?<Grid container>
                <Grid  xs={12} sm={12} md={6}>
                    <div className = "x-nft-item-image">
                        <video autoPlay loop muted alt = "nft-item" width = "100%" style = {{borderRadius: "5px",}}>
